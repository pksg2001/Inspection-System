...
from .forms import NewUserForm, InvoiceForm, InvoiceSearchForm, InvoiceUpdateForm
from .models import *
...
def list_invoice(request):
	title = 'List of Inspection Items'
	...
	if request.method == 'POST':
		...
		if form['generate_invoice'].value() == True:
			...
			def generate_invoice(name, invoice_number, 
				...
				# else:
				c.drawCentredString(400, 660, 'Inspection Token' + ':')
				c.setFont('Helvetica', 12, leading=None)
				invoice_number_string = str('#000' + invoice_number)
				...
				c.setFont('Helvetica', 12, leading=None)
				c.drawCentredString(80, 660, "Vendor: ")
				c.setFont('Helvetica', 12, leading=None)
				...
				c.drawCentredString(310, 580, 'Inspection System')
				...
				c.drawCentredString(450, 520, 'Unit Total')  
